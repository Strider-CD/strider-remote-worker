...
Drone.prototype = {
  attachListeners: function () {
    ...
    this.jobio.on('job:queued', function (id, timestamp) {
      ...
      if (!job) return this.error('queued event for unknown job', id)
      job.queued = timestamp
    })
    this.jobio.on('job:started', function (id, timestamp) {
      var job = this.jobmap[id]
      if (!job) return this.error('started event for unknown job', id)
      job.started = timestamp
    })
    this.jobio.on('job:tested', function (id, code, timestamp) {
      var job = this.jobmap[id]
      if (!job) return this.error('tested event for unknown job', id)
      job.testCode = code
      job.testTime = timestamp - job.started
    })
    this.jobio.on('job:deployed', function (id, code, timestamp) {
      var job = this.jobmap[id]
      if (!job) return this.error('deployed event for unknown job', id)
      job.deployCode = code
    ...
    this.jobio.on('job:*', function () {
      this.io.emit('browser', this.event, [].slice.call(arguments))
    })
